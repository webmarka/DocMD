            ...
            for file in files:
                if file.endswith(".md"):
                    ...
                    if not should_exclude(file_path, exclude_paths):
                        # Chemin relatif par rapport au dossier parent pour le lien HTML
                        rel_path = file_path.relative_to(base_path.parent).with_suffix(".html")
...
def convert_md_to_html(md_file_info, output_dir, all_pages):
    ...
    html_output = template.render(
        ...
        lang=LANG,
        pages=all_pages,           # Liste des pages pour la navigation
        current_page=md_file_info["rel_path"]  # Page actuelle pour surligner
    )
...
def generate_site():
    ...
    md_files = scan_markdown_files(INCLUDE_PATHS, EXCLUDE_PATHS)
    if not md_files:
    ...
    print("\nWrite HTML files.")
    for md_file in md_files:
        convert_md_to_html(md_file, OUTPUT_DIR, md_files)
