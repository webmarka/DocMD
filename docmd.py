...
def scan_markdown_files(include_paths, exclude_paths):
  """Scans folders for Markdown files."""
  markdown_files = []
...
def convert_md_to_html(md_file, output_dir):
  ...
  html_output = template.render(title=md_file.stem, content=html_content, lang=LANG)
...
def generate_site():
  ...
  for md_file in md_files:
      save_md_file(md_file, save_dir)
  # Write HTML files.
  print("\nWrite HTML files.")
  for md_file in md_files:
      convert_md_to_html(md_file, OUTPUT_DIR)
...
def directory_security_check(DIRECTORY):
  ...
  if (
        ...
        and not DIRECTORY.is_absolute()  # Pas un chemin absolu (remplace startswith("/"))
